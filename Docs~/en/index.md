...
| **`Invoke`** ⭐ | [Cmd_Invoke](API/UCL_AgentCommand/Cmd_Invoke.md) | **Generic reflection invocation** — fire any Unity public static method / property / field dynamically from a string description (type / member / args), no dedicated Cmd required per API; parsing + execution extracted into `UCL.Core.UCL_ReflectionInvoker` (located in UtilCore, runtime-available, callable from sources beyond Cmd), Type resolution goes through the shared `AssemblyExtensions` cache |
