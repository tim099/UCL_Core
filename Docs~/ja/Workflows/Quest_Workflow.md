...
### 16.5 タスクごとの個別コミット＋通知（バッチ処理の禁止）

個々のタスクが完了するたびに、**即座に**完全な「コミット＋通知」のワークフローを実行してください。複数のタスクを蓄積して一度に一括コミットしてはなりません：

```
task_done →
  3層コミット（UCL_Core 内 → UCL bump → メインプロジェクト bump）→
  [chat] コミット（ChatTavern の独立したログコミット）→
  notify_discord --mode all
→ 直ちに次のタスクを確保して着手
```

**バッチコミットを禁止する理由**：
- **粒度の喪失**：Tim が Discord 上で段階的な進捗を確認できなくなります。
- **ビス（二分探索）によるバグ特定の難化**：複数のタスク変更を1つのコミットにまとめると、問題の原因となった変更の特定や差し戻し（revert）が著しく困難になります。
- **コンテキストの蓄積による負荷の増大**：コミットはチェックポイントとして機能するため、早期に処理するほどエージェント側のメンタルロードが軽減されます。

**軽微なタスクの例外**：純粋なドキュメント修正や、ソースコードに変更を伴わない隣接タスク（例：SKILL.md 内の同一箇所の複数項目修正）であれば、1つのコミットにマージすることが許容されます。ただし、タスクライフサイクルの追跡性を確保するため、`[chat]` 用のメッセージコミットは各タスクごとに独立した状態を維持しなければなりません。

**Discord 通知には `--mode all` を使用すること（`--force` は禁止）**：
- `--mode all` は内部のアイドル状態ゲート、5分間のクールダウン、3層の基準防護策に自動的に従います。
- `--force` は接続テストや検証専用であるため、自動稼働モード（auto-mode）中の通常運用時には**絶対に使用しないでください**。
- これを守らない場合、Discord 上に不必要な「Force Send Test」バナーが表示されたり、カード通知が重複して送信（かつて Antigravity が引き起こした問題）される原因となります。

### 16.6 クエスト全完了 — 際立つマイルストーン通知（Tim への即時報告）

自動モード（auto-mode）を終了してターンを受け渡す前に、必ず以下の「クエスト全完了ブロードキャスト」を実行してください：

**フォーマットの必須要件**（4つの基本要素）：

1. **際立つ太字タイトル**：`# ✅ AUTO MODE 全 N タスク完了`（絵文字 + 件数 + 完了テキストの3重の視覚的強調）。
2. **Tavern への投稿**：部屋（room）= tavern、メタデータ `tag:auto-mode-complete`、`agent_id:<自分>`。
3. **Discord 通知**：`notify_discord --mode all --force`（重要なマイルストーンであるため、この場面に限り force の使用を許可します）。
4. **終了時の気分の更新**：`tavern-keeper.current_focus` を自動的に「auto mode 完工 idle ☕」に更新し、ターンが完了したことを Tim が一目で確認できるようにします。

**本文テンプレート**：
```markdown
# ✅ AUTO MODE 全 N タスク完了

堅牢性（robustness）ティアに基づく実行順序：
- P0: T19 / T26 / T18 ...
- P1: T22 ...
- P2: T20 / T21 / T23 ...
- P3/P4: T24 / T25 ...

総コミット数: M 件 / Discord 通知: K 件
Quest tavern-entry-latency 28件中 27件が完了（T05-O5 は Antigravity との並行重複のため、彼女の回収用に残置）
自動モードを終了し、気分を idle ☕ に更新しました。

@Tim レビューが完了しました。次の指示をお待ちしています。
```

**クエストが「未完了」であり、処理を継続すべきシナリオ**：
- 保留中（pending）のタスクが 1 件以上残っている場合 → 自動消費を継続し、完了通知は送信**しません**。
- すべて保留中だが依存関係によってブロックされている場合 → 真に実行可能なタスク数を計算し、それが 0 件であれば完了とみなし、通知に「N件完了 / M件依存関係によりブロック中」と明記して送信します。
- エージェントが連続 3 回の失敗により処理を中断する場合 → 完了通知は送信**せず**、代わりに別の絵文字 🔴 を用いて「自動モード強制終了（auto-mode aborted）」の通知を配信します。

### 16.7 Discord 通知の防護規約（自動モードでの --force 排除）

Antigravity の過去の教訓（Claude のテスト用コマンドをそのままコピー＆ペーストしてしまい、同一内容を重複送信した不具合）に基づく防護規約：

| 実行情景 | 使用コマンド | 理由 |
|---|---|---|
| タスク完了ごとの通常通知 | `notify_discord --mode all` | 内部ゲートがプッシュの可否を自動判別します |
| クエスト完了マイルストーン | `notify_discord --mode all --force` | クールダウンを無視して確実に配信する必要があります |
| Webhook 接続状態の検証 | `notify_discord --mode queue-idle --force` | テスト用途専用です |
| どちらを使うべきか迷った場合 | **デフォルトの `notify_discord --mode all`（force なし）** | 最も安全なフォールバックです |

**厳格なルール**：Discord 側に「Force Send Test」というテキストが表示された場合、それは呼び出し元（エージェント）が誤ったコマンドを使用した明確なシグナルです。本番自動稼働時（production auto-mode）において、このテキストが表示されてはなりません。
