...
## 🚀 3. 増分追跡とタグのイテレーション (Incremental Tracking & Tagging)

> 頻繁な Git Commit の中で迷子にならないよう、私たちは「Localization Checkpoint」タグメカニズムを導入します。これにより、すべての変更が整然と消化され、翻訳されていない死角を残さないようにします！

### 3.1 イテレーションサイクル SOP

一括ローカライズの収穫準備が整ったら、次の神聖なステップに従って順次実行してください：

1. **🔍 アンカーの探索 (Find Anchor)**：
   `git tag` を使用して、形式が `Localize_{N}`（例：`Localize_01`）である直近のタグを探します。
   - 存在しない場合は、そのファイルの最初のコミットまたは Initial Commit を開始点とします。
2. **📑 変更の取得 (Fetch Changes)**：
   `git diff --name-only <Last_Tag> HEAD` を実行し、`Docs~/zh-Hant/` ディレクトリ配下で変更されたすべての Markdown ファイルを抽出します。
3. **⚙️ 翻訳の実行 (Process Files)**：
   これらの変更されたファイルに対して、**§2. SOP ドキュメント翻訳の5ステップ** に従い、対応する言語の更新と翻訳を1つずつ完了させます。
4. **📦 コミットとタグ付け (Commit & Tag)**：
   - まず、翻訳されたファイルを Stage して Commit します。
   - 「増分ナンバリング」または「タグ上書き」戦略に従って、新しいタグを付与します。

### 3.2 タギング戦略 (Tagging Strategy)

本お嬢様は、ここに2種類のタグ処理スタイルを承認します：

| 戦略名 | 適用シーン | Git 操作例 | 備考 |
| :--- | :--- | :--- | :--- |
| **🏰 高雅なナンバリング派 (Versioning)** | **推奨**。すべてのローカライズ履歴の軌跡を保持し、事後の追跡を容易にします。 | `git tag Localize_02` | 本お嬢様が最も愛する歴史的継続感ですわ！✨ |
| **🧹 怠惰な上書き派 (Moving Tag)** | 「現在の最新進捗」のみに関心があり、タグリストを膨張させたくない場合。 | `git tag -d Localize_01` <br> `git tag Localize_01` | 野暮ですが有効ですわ。実行する前に、アンカーを失う懸念がないことを必ず確認すること！ふん！ |

---

## ⚠️ 4. よくある落とし穴 (Common Pitfalls)
