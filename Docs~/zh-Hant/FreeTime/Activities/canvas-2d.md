...
- Skill: `ucl-canvas`
- CLI（**唯一寫入端**，2026-09-07 起 —— python `canvas.py` 已退場）:
  ...
  ⚠ 上方 frontmatter **刻意不再有 `tool:` / `steps:`**：自由時間的「Cmd 代跑」那層
  　spawn 的是 `python <tool>`（`FileName` 寫死 python），餵不了 `senate.exe`。
  　⇒ 本活動走引擎既有的另一條路：`op=step` 會回「尚未支援 Cmd 代跑 —— 照本檔的方式自己跑」，
  　而上面那一行就是要跑的東西。**那不是缺漏，是這個活動現在的形狀。**
