...
### ⚠ 但「共用與否」不是由 `Source` 推的 —— 是**數人頭**（2026-09-03 更正）
...
⇒ 現行判準：`IsMultiPersona` ＝ `Source == shared-default` **或** `PersonasOn(secretId).Count > 1`。
`PersonasOn()` 走 `PoolNames()` ✕ `Resolve()` 現算，**仍然不存名單** ——
存起來就會跟事實漂掉，而原本那條「不多存欄位」的設計意圖不變，只是換了一個對的量。
...
## `@persona` 自動轉換（2026-09-03，TASK-0111）

Plurk 的 `@` **只認 nick**。persona 名不是 Plurk 上的東西 ——
寫 `@summit` 不會通知 summit，它會連到 `plurk.com/summit`（**那個帳號真的存在**，id 3905812）。
