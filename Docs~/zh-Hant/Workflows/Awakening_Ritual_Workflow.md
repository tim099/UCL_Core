...
- **見人折人**（見林的**第一步**，Tim 2026-09-01 拍板／2026-09-05 再確認順序）：
  **先折人 → 再跑見林**，兩者同一趟做完，**折人不排自己的班**。
  ...
  ⓐ 根層還有未歸檔畫像 ⇒ 擋（`fold_gate=blocked_unfolded`）；
  ⓑ 折人跑完但 `digest_body` 一位同事都沒提 ⇒ 擋（`fold_gate=blocked_no_colleague`）。
  ⇒ **見林＝這段期間的心得 ＋ 這段期間對同事的看法，一起寫**（Tim 原話）——
  折人的產出要真的進到那一片林裡，跑完折人不等於做到了。
  出口：`--arg fold_skip_reason=<理由>`（補跑舊區間等），**非空即放行且理由留名**
  （回傳檔 ＋ `_cmd_results/<id>.json`）。⛔ 刻意不做成 `=1` 的布林旗標 ——
  一個不必寫理由的跳過，跟沒有閘一樣。
  🩸 為什麼從提示升級成擋：舊的提示只印在 `digest_body` **沒給**那條路（inspect），
  帶 body 直接寫入時一次都不印；而 brief §9 那份自 2026-09-06 起只在見林到期時才列
  ⇒ **補跑舊區間（gap < 門檻）＋直接帶 body ＝ 零提示**。
  ⚠ 見林還沒到門檻時，「N 位／M 幅未歸檔」是**讀數不是待辦**——
  它回答的是「見林那天要折幾幅」，不是「今天該去折」。
  🩸 只寫「是見林的一部分」擋不住這個誤讀：2026-09-05 basecamp 就把 brief §9 那一行
  讀成一條「不卡別人、可以先做掉」的獨立線，而當天見林 gap 是 9/10。
  見林時把 `sketchbook/` 根層**未歸檔的畫像全折完**，
