...
- [ ] **會重畫的宿主，開真視窗轉十秒** —— 不是「跑一次沒報錯」，是讓它連續重繪
  ...
  - ⭐ **十秒是下限，不是一道額外的儀式**（Tim 2026-09-15 拍板）：build 完的視窗**常駐開啟**（設計如此，不是意外）
    ⇒ 日常使用早就遠超過十秒，受測體**不必刻意湊** —— 這一格真正要防的不是「沒跑滿十秒」，
    是**拿文字／headless 宿主的無感當通過**。
    🩸 血證（kiara，2026-09-15）：我在 TASK-0178 上推論「唯一符合的受測體早修好了，要湊只能刻意把一支頁改回每幀直呼子程序」
      —— 而那整段建立在一個我沒去量的前提上：**我以為真視窗是要特地開起來的東西。**
      ⇒ 窗口邊界被我寫成了機制宣告，而它讓這張單多滾了五天。
  - ⭐ **量法與門檻（沒有這一格，上面那句話只是願望）**：
    - 量：`senate ui --soak <秒> --page <頁>`（旗標今天仍在）
    - 門檻：**10 fps**。它抓的是**凍住**不是**慢** —— 門檻設高會變成會自己叫的假警報，而**假警報最後一定是被關掉，不是被修好**。
    - 🩸 **第一幀要分開印，不合併也不省略**（`第一幀 X ms，其餘最慢 Y ms`）——
      把第一幀當噪音排除掉的那一版，對一個**凍住的視窗**印出來的是「最慢一幀 0.0 ms」。
      ⇒ **排除噪音的動作會連訊號一起排除掉，而排掉的東西不會有人喊。**
    - 受測體不必自己想：**D21 那張表已經有兩個值不同的**（`--page home` 235 幀/4.01s 過／
      `--page submodule` 1 幀/8.71s ⇒ 0.1 fps 擋）。⚙ 位址：Senate `Docs/Logs/Decisions.md` **D21**（2026-09-03）。
      ⚠ 那是 09-03 的讀數，**讀數有時刻** —— 要簽這一格就自己重跑一次，不要引用舊的。
      🩸 **而「重跑一次」今天就證明了為什麼非重跑不可**（kiara 2026-09-15 實跑）：
      `--page submodule` 現在是 **597 幀/10.01s ⇒ 59.6 fps、第一幀 33.7 ms ⇒ 過** ——
      它 09-03 擋的那個原因已經被修掉了（Senate `d40f7c9`「Submodule 頁掃描搬進背景」）。
      ⇒ **照抄 D21 那張表去挑受測體的人，會挑到一支今天全綠的頁，而它看起來跟驗過了一模一樣。**
    - 📌 而它 2026-09-04 已經**退出 build 的自動閘**（Tim 拍板，換成常駐視窗）
      ⇒ 現在擋它的是**人的眼睛**：build 收尾那顆窗就在你面前。
      位址：Senate `Docs/Workflows/Setup_And_Build.md`（「退場的那一格」那段）。
  - ⚠ **而「兩個值不同」不保證在同一個時刻找得到兩支頁**：
    2026-09-15 量到的是 `home` 60.0 fps、`submodule` 59.6 fps —— **全樹當下沒有一支會擋**。
    ⇒ 這一格真正要的不是「湊出一支壞頁」，是**證明這把尺說得出「不」**：
    同一支 `submodule` 在 09-03 是 0.1 fps（擋）、在修掉之後是 59.6 fps（過），
    **同一把尺、同一支受測體、兩個不同的判定** —— 那就是它不是「怎麼跑都綠」的憑據。
    ⛔ 找不到當下會擋的頁時，**不要為了湊而弄壞一支頁**（代價落在共用環境上）；
    去翻這把尺歷史上擋過誰，那份讀數跟現跑的一樣算數。
