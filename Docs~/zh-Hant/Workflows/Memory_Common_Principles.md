...
| 問題的形狀 | 照 §1 判準它住在 | 該查 |
|---|---|---|
| 「我是誰／我反覆犯什麼／我對這件事的看法」 | 個人記憶 | `frag_<persona>,alaya` |
| 「**跟某人做的某件作品**／某個主題當初怎麼拍板」 | **工作記憶／閱讀庫** | `all`（或直接 `work_memory,library`） |
| 「這條通用守則是什麼」 | Alaya | `alaya` |

🩸 **血證（summit 2026-09-11，同一個問題、只改 target）**：查「我跟 Sirius 一起做的漫畫改編」——

| 輪 | target | 最高分 | 撈到什麼 |
|---|---|---|---|
| 1 | `frag_summit,alaya` | 0.6235 **灰帶** | **另一個專案**（跟 gura 的漫畫改編） |
| 2 | `frag_summit,alaya`（換句話再問） | 0.6063 **灰帶** | 同上 |
| 3 | **`all`** | **0.6693 / 0.6448 / 0.6333** | ✅ 真命中，全在 `work_memory` 與 `library` |

⇒ 答案**本來就不該在碎片層**（它綁一件具體工作）。兩輪不是方法錯，是**範圍錯** ——
而灰帶那兩輪還遞了一個**近似但不是那件事**的結果回來，那比空手更容易被當成答案。

📌 所以讀灰帶時要先分辨它是哪一種（這決定下一個動作完全不同）：
...
🔑 **而語意檢索的最後一個出口是離開語意檢索**：§4 開頭那句警語說「查不到跟不存在同形」，
它的解不是再換一句話問，是**換一個不同源的量具** ——
去磁碟 `grep` 第一手（2026-09-11 同一件事：`grep -rl 十八天` 一次命中，而且順帶印出
「誰在講它」的目錄分布）。⭐ 語意檢索答「像不像」，`grep` 答「在不在」，**兩個不同的問題**。
...
1. ~~**不能只查某一個 persona 的記憶。**~~ ⇒ ✅ **已解：用單 persona 索引 `frag_<persona>`**
   （由 config 依磁碟自動展開，實測 summit 兩次命中；`ucl-memory` §1 有用法）。
...
⛔ **而它有一個排除條件（summit 2026-09-11 補）**：先確認正解**照 §1 判準本來就該住在你查的那一層**。
若灰帶的成因是「target 圈錯層」（撈到別的專案／整排沾到邊），**那不是回填的訊號，是換 target 的訊號** ——
照字面回填下去，等於把一條綁工作的記憶硬塞進碎片層，而那會同時壞掉兩件事：
碎片層多一筆不屬於它的內容，工作記憶那筆**仍然沒有人找得到**。
⇒ 兩種灰帶的分辨表在 §4「先選對層」。
