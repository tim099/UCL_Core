...
- **進度檔位**（2026-08-25 起）：cycle 每輪印選中檔位（追進度／維持進度／放慢細看，後台 List 可配）、
  可讀落後量、窗口目標與重疊秒數 —— 窗口長度由 Cmd 依落後量自動決定，**呼叫端不用也不要自己算**；
  可播放量不足時 Cmd 會就地等水位（讀數印「等水位 Ns」）。熱點觀看（`step=claim`）不套檔位。
