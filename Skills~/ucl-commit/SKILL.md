...
## 訊息裡要有「順手修掉的（Q0）」那一段（Tim 2026-08-25 拍板）

做一件事的過程中順手修掉的**實作細節**，判準見 `ucl-task` skill §0.5 的四階梯 Q0 ——
**它不上任務單，但它的痕跡落在這裡。**

> **「不佔單子」不等於「不留痕跡」。**
> Q1／Q2／Q3 都有天然落點（單上留言／驗收細項／單子本身），
> **只有 Q0 沒有，而 Q0 是每天觸發最多次的那一階** ⇒ 沒有落點它就是隱形工作的入口。
> （@kiara 2026-08-25 指出這個洞）

### 怎麼寫

commit 訊息裡開一段，**寫「為什麼」與「它會怎麼咬人」，不要只寫「改了什麼」**：

```
## 順手修掉的（Q0）

<這個東西原本會怎麼咬人 —— 一句話>
<為什麼它不上單子：四個角色都不需要在單上討論它>
<如果有血證，寫現場讀數>
```

⚠ **不要寫成 changelog**（「改了 A、改了 B」）——
那種訊息三個月後 `git log --grep` 撈到了也看不出該不該回頭看它。
✅ 要寫**它為什麼存在**：`git log` 是 Q0 唯一的檢索面。

📌 為什麼落點是 commit 而不是文件：
做的人**本來就要 commit**（成本零）／訊息**跟那個 diff 綁死**（不會自己漂走）／
**可以被 grep**。文件會漂、記憶會歸檔，而 commit 訊息不會。
