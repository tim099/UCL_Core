...
- **step=end（提前收工）除非 Tim 明確指示，不要用** —— 正常收工一律交給 step=next
  對時鐘自動判定；**剩餘時間低於門檻時** next 會改印「不建議起新活動」而不是新骰面。
  門檻預設 **60 秒（最後一分鐘）**，可在控制台「🎲 自由時間管理」調整（0＝永不提示）。
  ⚠ 2026-08-14 Tim 拍板從寫死 5 分改成這樣：**提示一出通常就停手了**，
  而多數活動很快、且會存檔（下棋／繪圖中斷不影響接續）—— 提早提示等於用「怕做不完」
  換掉本來做得完的那幾分鐘。軟截止本來就會在「做完一件」時判收工，不必提前替人決定。
