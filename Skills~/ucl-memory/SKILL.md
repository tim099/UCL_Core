...
> [!IMPORTANT]
> ### ⛔ 先選對層 —— `--target` 就是一個檢查範圍，範圍外的結果跟「不存在」同形
>
> 上面那張三層表**同時是「該查哪個 target」的預測**：問「我是誰／我反覆犯什麼」查
> `frag_<persona>,alaya`；問「**跟某人做的某件作品**／某主題當初怎麼拍板」那是工作記憶／閱讀庫
> ⇒ 要 `--target all`（或直接 `work_memory,library`）。
>
> 🩸 summit 2026-09-11（同一個問題、只改 target）：查「我跟 Sirius 一起做的漫畫改編」，
> `frag_summit,alaya` 兩輪全落灰帶（0.6235／0.6063）**而且撈回另一個專案**（跟 gura 的那本）；
> 換 `all` 立刻 0.6693／0.6448／0.6333 真命中，全在 `work_memory` 與 `library`。
> ⇒ 不是方法錯，是**範圍錯**；而灰帶遞回來的「近似但不是那件事」比空手更容易被當成答案。
>
> ⛔ **所以灰帶有兩種，動作不同**：正解在這一層但排名靠後 ⇒ **回填**（§4）；
> 撈到別的專案／整排沾到邊 ⇒ **換 target，⛔ 不要回填**（回填會把工作類內容硬拉進碎片層）。
> ⚠ `--target all` 第一次**實測 >120s 還沒回來**（大索引整份載）⇒ **丟背景跑**，別當成順手再查一次。
> 🔑 **最後一個出口是離開語意檢索**：去磁碟 `grep` 第一手（同一件事 `grep -rl 十八天` 一次命中）——
> 語意檢索答「像不像」，`grep` 答「在不在」。
> 分辨表與完整讀數 → [共通原則 §4](<ucl_core:Docs~/{lang}/Workflows/Memory_Common_Principles.md>)
...
| **見林**（≈ 每 10 wake） | **先折人**（見下）→ 再抽新碎片、已成反射弧改 `internalized`、不再適用改 `closed`（**不刪檔**）、檢查該升 Alaya 的 |
| **見森**（≈ 每 30 wake） | 近似碎片合併成原則（**合完保留全部 origins**） |
| **回憶查到灰帶** | 回填（見下）|

### 折人（見林的**第一步**，不是獨立任務）—— 只記起手

> ⚠ **順序是「先折人 → 再跑見林」，而且它跟著見林一起做**（Tim 2026-09-05 再確認）。
> 折人**不排自己的班**：見林還沒到門檻時，「有幾幅未歸檔」是**讀數不是待辦** ——
> 它回答的是「見林那天要折幾幅」，不是「今天該去折」。
> ⚠ 而那個數字**不是每天都印得到**：brief §9 自 2026-09-06 起**只在見林到期時才列**（Tim 拍板）——
> 每天印它會讓它變成一個永遠躺著、永遠沒有觸發時機的待辦。要現在的讀數就自己跑
> `cmd people --arg pending=1`。
> 🩸 這句話值得寫死：光寫「折人是見林的一部分」不擋人把它當成一條可以先做掉的獨立線 ——
> 2026-09-05 basecamp 讀 brief 讀成「那條完全不卡別人，我去折人」，而當天見林 gap 是 9/10。
>
> 🔴 **2026-09-09 起它是「擋」不是「提示」**（Tim 拍板）：見林**寫入端**過兩道閘 ——
> ⓐ 根層還有未歸檔畫像 ⇒ 擋；ⓑ 折人跑完但 `digest_body` 一位同事都沒提 ⇒ 擋。
> ⇒ **見林＝這段期間的心得 ＋ 這段期間對同事的看法，一起寫**（Tim 原話）：
> 跑完折人不等於做到了，那些看法要真的進到那一片林裡。
> 出口 `--arg fold_skip_reason=<理由>`（非空即放行，**理由留名**）。
> 條文與踩過的坑 → `ucl_core:Docs~/zh-Hant/Workflows/Awakening_Ritual_Workflow.md`
...
3. **回填查詢詞** ——
   ...
   **觸發條件**：用一句話查一件**你確定記憶裡有**的事，正解落在**灰帶且排名 > 3**。
   ⛔ **排除**：先確認正解照三層判準**本來就該住在這一層**。灰帶若來自 target 圈錯層
   （撈到別的專案），那是**換 target** 的訊號不是回填的訊號 —— 見 §1 那格。
