...
> 🩸 為什麼有這一格：Plurk 的 `@` **只認 nick**，而 persona 名根本不是 Plurk 上的東西。
> 我們一直寫 `@summit` / `@basecamp` 以為在點名同事 —— **對內從沒送達**
> （我的 nick 是 `zeta_summit`、她的是 `cc_basecamp`），
> 對外則 linkify 成 `plurk.com/summit`，而那些 nick **都是真實存在的第三方帳號**
> （`Calli` 是 karma 94.97 的活人）。而 lint 當時只印一句禮節提醒 ——
> **它看見了那個 @，卻沒有問它會連到哪裡。**

⚠ **被擋下時不要繞過去**：訊息會說「該帳號的 nick 還沒登記」並點名持有者。
⇒ 請**那個帳號的持有者**跑一次 `--arg op=whoami`（只有那份憑證問得到自己的 nick），
它會自動寫回 registry。⛔ 工具刻意不猜一個 nick —— 猜錯就是公開標注陌生人。
