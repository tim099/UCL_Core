...
|---|---|---|
| **各集獨立**（多數） | 目前最大章號 **+1**（＝章數 +1，見下方不變式） | **不需要 catchup**，直接 `step=join` |
...
> ⛔ **不變式：章號必須連續，不准有洞**（Tim 2026-08-27 拍板：「最大章號應該要等於目前章數」）。
> ⇒ `1,2,3,4` ⇒ 最大 ＝ 4 ＝ 章數 ⇒ **「最大 +1」與「章數 +1」永遠是同一個答案**。
> 📌 這條不變式是那兩句話**得以互換的前提**，而它此前沒有寫在任何地方 ——
> 一旦出現跳號（例：只有 `001`、`003`），兩句話會給出不同的號，而**其中一句會再撞一次號**。
> ⇒ 給號前先確認實錄章沒有洞；**有洞先補洞，不要繞過它往上加**。
...
## 🕐 primary 收工那一步**會逾時，而那是預期**（TASK-0176，Tim 2026-09-08 拍板）

收尾（實錄匯出＋收工關錄影）由**主觀影者**觸發，而她收工後 Cmd 會**再等最多 120 秒**
讓同場的人收播（每 5s 回讀 session 檔；全員到齊就提早返回）。到期仍有殘留 ⇒ **強制結算**那幾場。

⇒ **`senate ucmd` 預設只等 120s** ⇒ primary 的收工這一步**幾乎必然** `exit 3` ＋ `delegate_failure = timeout`。

> ⛔ **逾時 ≠ 沒執行 —— 不要重打。**
> 處置：**看回傳檔的 mtime**（收尾跑完它會更新），或 `senate cmd sessions` 回讀那幾場的 `end_reason`。
> 🩸 重打的代價是可數的：強制結算與**收播公告會再發一次**（而公告帶發薪）。

📌 為什麼判準從「最後一個收工的人」換成 primary：舊判準要求**每一個人都回來**，
而有人沒回來時它的失效樣子是**沉默** —— 2026-09-08 實撞：四個人的回傳檔都印「還不是最後一個」，
於是沒有人成為最後一個 ⇒ **章沒進書、錄影整晚開著**（那晚是 Tim 手動關的）。
⚠ 而兩個消費者的代價**不對等**：章沒進書可事後 `op=export` 補；**錄影沒關會一直錄下去**。
⇒ 現在是「指定一個負責人 ＋ 一個時限 ＋ 到期強制回收」——不確定性被**有界化**。
