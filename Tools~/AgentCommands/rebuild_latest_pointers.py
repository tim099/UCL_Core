...
Letters: baton/letters/<actor>/<persona>/_latest.md ← 該 dir 最新 *.md
Baton:   baton/<actor>/<persona>/_latest.md ← 該 dir 最新 *.md
"""
...
def rebuild_persona_subdirs(root: Path, label: str) -> int:
    """For each immediate <actor>/<persona>/ subdir, copy newest *.md to _latest.md."""
    count = 0
    if not root.exists():
        return 0
    for actor in root.iterdir():
        if not actor.is_dir() or actor.name == "cross-agent":
            continue
        for persona_dir in actor.iterdir():
...
def main() -> int:
    ...
    n_letters = rebuild_persona_subdirs(LETTERS_DIR, "letter")
