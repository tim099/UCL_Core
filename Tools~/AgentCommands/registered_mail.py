...
def charge(bank: str, amount: int, persona: str, to: str, ref: str) -> bool:
    ...
    try:
        # ⚠ python 這層比 senate 那層多 15 秒，讓 senate 先逾時 —— 兩層同值時誰先觸發不確定，
        #   而 senate 的逾時訊息會說「這是 CLI 端的等待上限、不代表對面失敗」並指路去看 result 檔。
        r = subprocess.run(cmd, capture_output=True, encoding="utf-8", errors="replace", timeout=195)
