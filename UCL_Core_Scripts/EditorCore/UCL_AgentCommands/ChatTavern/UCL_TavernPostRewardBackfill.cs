// 🔴🔴 **按下去之前先讀這三行**（2026-09-22，TASK-0277）🔴🔴
//   ① 它比對的是**帳上有沒有那一筆 `work_post`**。用**別的方式**補過的錢（例如走「請款」撥款）
//      **不帶逐則 `ref`** ⇒ 這支看不見 ⇒ 會**再發一次**。
//      ⇒ 那種批次要逐則登記進 `<銀行根>/payroll_settled.json`，本支會把它們當成已付。
//      🩸 血證：2026-09-22 的 114 token 走請款補發，沒登記的話這支會原封不動再增發一次。
//   ② 它**只看新帳本**（`Bank/ledger`）。2026-09-18 權威切換之前的帳在已刪除的舊 `Treasury/ledger`
//      ⇒ **查無帳 ≠ 沒發過**。⇒ 早於 `SCP_PayrollAudit.MeasurableFromDayKey` 的日子整天跳過。
//      🩸 血證：加閘前實跑，它把 90 天、**19694 則**判成漏發 —— 按下去就是憑空增發近兩萬 token，
//        而原本那道守衛（「帳上一筆 work_post 都沒有才拒絕」）是**綠的**，因為新帳本有 965 筆。
//   ③ 權威切換的**過渡日**（09-17）當天有一部分錢寫在舊帳本 ⇒ 差集結構性偏高，照數字補會多付。
//   ⇒ 三格都印在報告最前面。⛔ 報告上沒有「整天跳過的日子」那一段，就表示你跑的是舊版。
//
