...
namespace UCL.Core.EditorLib.AgentCommands
{
    ...
    public class Cmd_SessionStatus : UCL_AgentCommandHandlerBase
    {
        ...
        public override async UniTask ExecuteAsync(Dictionary<string, string> args, CancellationToken token)
        {
            ...
            string aOut = Path.Combine(aOutDir, "_session_status.md");
            File.WriteAllText(aOut, aR.ToString(), new UTF8Encoding(false));
            Debug.Log($"[SessionStatus] → {aOut}");
            UCL_AgentCommandRunner.ReportOutputFile(args, aOut);
        }
    }
}
#endif
