    ...
    public static class UCL_TreasuryAccountResolver
    {
            ...
            try
            {
                if (File.Exists(RegistryMetaPath))
                {
                    ...
                    if (reg != null && reg.IsObject)
                    {
                        ...
                        // 🩸 `bank_personas` 反向表**退出解析**（Tim 2026-09-07 拍板：
                        //   `letters/<persona>/bank/<region>.md` 才是權威版本）。
                        //   它仍然留在 `_registry_meta.json` 裡沒有被刪，但**沒有人再讀它** ——
                        //   健檢改走 `senate cmd bank-audit`（那支會把「這個欄位還在」報成待清理）。
                        //
                        //   為什麼不是「換個優先序」而是整段拿掉：它**沒有寫入端**，
                        //   而一張沒有寫入端的表只會愈來愈舊。
                        //   血證：2026-08-20 `Sirius` 的帳戶改名 `Federal Reserve System` → `FRS`，
                        //   綁定檔跟著改了、反向表沒有 ⇒ **它錯了 18 天而沒有任何一層喊**。
                        //   （沒有釀成事故只是因為 ⓪ 合一那一跳先命中 —— 那是**沒有人踩到**，
                        //    不是那條路是對的。）
                        //   ⇒ 同一 persona 兩家 bank 的「撞名拒絕解析」也跟著退場：
                        //     一區一檔的正向綁定讓那個狀態**在型別上不可能存在**，
                        //     而把錯的選項拿掉比留一道守衛好。
                    }
                }
            }
        ...
        public static TreasuryAccountResolution Resolve(string accountId)
        {
            ...
            lock (s_Lock)
            {
                ...
                // ⑤ persona → agent → bank 的 legacy 兩跳鏈。
                //    ⚠ 走到這裡代表 ⓪ 沒命中，也就是**這個 persona 在本區沒有綁定檔**
                //      （`letters/<p>/bank/<region>.md` 不存在，連跨區借用都沒有）——
                //      而綁定檔是唯一的權威（Tim 2026-09-07）。
                //    ⇒ 那麼「他屬於哪家 bank」這件事，系統其實**不知道**；
                //      這一跳只是拿舊的 `agent_banks` 猜一個看起來合理的答案。
                //    📌 所以它的 Trace 必須說出自己是猜的，否則它與 ⓪ 的權威答案在報告裡同形。
                    ...
                    && s_AgentToBankLower.TryGetValue(agentOfPersona.ToLowerInvariant(), out var bankViaPersona))
                {
                    ...
                    r.Trace = $"⚠ persona `{accountId}` → agent `{agentOfPersona}` → bank `{bankViaPersona}`"
                            + "（legacy agent_banks 兩跳；**此人沒有本區綁定檔**，"
                            + "權威來源缺席 ⇒ 這是推導不是宣告）";
                    WarnLegacyAgentBanksHop(accountId, bankViaPersona, "⑤ persona→agent→bank");
                    return r;
                }
