    ...
    public static partial class UCL_CodeLocalize
    {
        static public string Get_ja(string iKey)
        {
            return iKey switch
            {
                ...
                "MissingRef.Title" => "Missing Reference 調査",
                "MissingRef.Intro" => "二種類の破損を走査：<b>⚠ 壊れた参照</b>（instanceID は残っているがオブジェクトは削除済み —— クリアすれば戻せる）と <b>🔴 スクリプト欠落 Component</b>（m_Script が null —— 丸ごと削除するしかなく<b>不可逆</b>）。",
                "MissingRef.NotThis" => "⛔ <b>本ページは Odin/PropertyEditor の NRE を直すとは主張しない</b>（<i>InspectorConfig.UpdateOdinEditors → ClearEditorsAndRebuild</i>）。あれは Unity 内部で落ちており、よくある原因は「Inspector が追跡中のオブジェクトにスクリプト欠落 Component がある」こと。⇒ まず 🔴 を消してから再現するか確認を。消えなければ原因はこれではない。",
                "MissingRef.Scope" => "走査範囲",
                "MissingRef.Scope.Scenes" => "開いているシーン（未保存の変更を含む）",
                "MissingRef.Scope.Prefabs" => "全 Prefab（t:Prefab —— 大規模プロジェクトでは数十秒）",
                "MissingRef.Scope.ScriptableObjects" => "全 ScriptableObject（t:ScriptableObject）",
                "MissingRef.Scan" => "🔍 走査",
                "MissingRef.NoTarget" => "⚠ <b>対象が一つも走査されなかった</b> —— これは「綺麗」ではなく、範囲が未選択（またはシーン未オープン）という意味。",
                "MissingRef.CleanFmt" => "✅ 問題なし：対象 {0} 件／シリアライズ済みオブジェクト {1} 件を走査、検出ゼロ。",
                "MissingRef.FoundFmt" => "<b>⚠ {0}</b> 件の壊れた参照、<b>🔴 {1}</b> 件のスクリプト欠落 Component を検出（対象 {2} 件／オブジェクト {3} 件）。",
                "MissingRef.ListFmt" => "結果（{0} 件）",
                "MissingRef.TruncatedFmt" => "…先頭 {0} 件のみ表示、全 {1} 件（残りは Console）。",
                "MissingRef.Kind.MissingScript" => "スクリプト欠落 Component",
                "MissingRef.Select" => "選択",
                "MissingRef.Clear" => "フィールドをクリア",
                "MissingRef.RemoveComponent" => "Component を削除",
                "MissingRef.FixAllRefsFmt" => "🧹 壊れた参照をすべてクリア（{0}）",
                "MissingRef.ConfirmClearFmt" => "{0} 件の壊れた参照を処理しますか？\n\n· 通常のフィールド ⇒ 空にする（可逆、再指定すればよい）\n· <b>配列／List の要素 ⇒ 要素ごと削除</b>（null にはしない —— null の穴も foreach は通るので、落ちる場所が変わるだけ）\n\nシーン／アセットは変更済みとしてマークされます。⛔ 自動保存はしません（Ctrl+S）。",
                "MissingRef.ConfirmRemoveFmt" => "この GameObject 上のスクリプト欠落 Component をすべて削除しますか？\n\n{0}\n\n⛔ 不可逆：それらにシリアライズされたデータも消えます。\n⚠ スクリプトが一時的にコンパイル不能なだけ（リネーム／assembly 未ビルド）なら、削除はデータを捨てることになります。",
