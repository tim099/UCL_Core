...
namespace UCL.Core.UI {
    /// <summary>
    /// A copy of UnityEngine.UI.Image just to figureout how it work and add some custom feature
    /// </summary>
    [RequireComponent(typeof(CanvasRenderer))]
    public class UCL_Image : MaskableGraphic, ISerializationCallbackReceiver, ILayoutElement, ICanvasRaycastFilter {
        static class UCL_SetPropertyUtility {
        ...
        static public UCL_Image Create(Transform parent) {
            var img = UCL.Core.GameObjectLib.Create<UCL_Image>("UCL_Image", parent);
        ...
        #region UCL_Extension
        void GenerateSpriteSheet(VertexHelper vh, bool lPreserveAspect) {
            var rect_transform = GetComponent<RectTransform>();
            var corners = new Vector3[4];
            rect_transform.GetLocalCorners(corners);
            var uv = (activeSprite != null) ? UnityEngine.Sprites.DataUtility.GetOuterUV(activeSprite) : Vector4.zero;
            if((m_Tile.x > 1 || m_Tile.y > 1) && m_Tile.x > 0 && m_Tile.y > 0) {
                int x = m_TilePos % m_Tile.x;
                int y = ((m_TilePos - x) / m_Tile.y) % m_Tile.y;

                float segx = (1f / m_Tile.x);
                uv.x = x * segx;
                uv.z = (x + 1) * segx;

                float segy = (1f / m_Tile.y);
                uv.y = y * segy;
                uv.w = (y + 1) * segy;
            }
            ...
            var color32 = color;
            vh.Clear();
            vh.AddVert(corners[0], color32, new Vector2(uv.x, uv.y));
            vh.AddVert(corners[1], color32, new Vector2(uv.x, uv.w));
            vh.AddVert(corners[2], color32, new Vector2(uv.z, uv.w));
            vh.AddVert(corners[3], color32, new Vector2(uv.z, uv.y));
        ...
        public enum DrawType {
            ...
            /// <summary>
            /// UCL Extension to draw image as sprite sheet
            /// </summary>
            SpriteSheet,
        }
        ...
        public Sprite overrideSprite {
            ...
            set {
                if(UCL_SetPropertyUtility.SetClass(ref m_OverrideSprite, value)) {
                    SetAllDirty();
                    TrackSprite();
                }
            }
        ...
        /// </code>
        /// </example>
        public int fillOrigin { get { return m_FillOrigin; } set { if(UCL_SetPropertyUtility.SetStruct(ref m_FillOrigin, value)) SetVerticesDirty(); } }
        ...
        /// </remarks>
        public bool useSpriteMesh { get { return m_UseSpriteMesh; } set { if(UCL_SetPropertyUtility.SetStruct(ref m_UseSpriteMesh, value)) SetVerticesDirty(); } }
        ...
        static List<UCL_Image> m_TrackedTexturelessImages = new List<UCL_Image>();
        ...
        private static void TrackImage(UCL_Image g) {
            if(!s_Initialized) {
        ...
        private static void UnTrackImage(UCL_Image g) {
